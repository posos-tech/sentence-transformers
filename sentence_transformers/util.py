...
def normalize_np_vecs(vecs, axis=-1):
    norm = (vecs**2).sum(axis=axis, keepdims=True)**0.5
    return vecs / np.where(norm == 0., 1., norm)


def numpy_cos_sim(a, b):
    assert isinstance(a, np.ndarray) and isinstance(b, np.ndarray)

    normalized_a = normalize_np_vecs(a)
    normalized_b = normalize_np_vecs(b)

    b_ndim = normalized_b.ndim
    return np.matmul(normalized_a, normalized_b.transpose(
        [b_ndim-1] + list(range(b_ndim-1))))
...
def paraphrase_mining_embeddings(embeddings: Tensor,
    ...
    for corpus_start_idx in range(0, len(embeddings), corpus_chunk_size):
        for query_start_idx in range(0, len(embeddings), query_chunk_size):
            ...
            scores_top_k_values, scores_top_k_idx = torch.topk(scores, min(
                top_k, len(scores[0])), dim=1, largest=True, sorted=False)
...
def snapshot_download(
    ...
    storage_folder = os.path.join(
        cache_dir, repo_id.replace(
            "/", REPO_ID_SEPARATOR) + "." + model_info.sha
    )
